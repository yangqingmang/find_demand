...
def main():
    ...
    # 测试默认词根
    print(f"\n🎯 默认激活词根 (前10个):")
    active_roots = manager.get_active_roots()
    for i, root in enumerate(active_roots[:10], 1):
        print(f"   {i}. {root}")
    
    # 测试手动指定母词
    print(f"\n✋ 测试手动指定母词:")
    manual_roots = ['generator', 'converter', 'analyzer', 'optimizer']
    manager.set_manual_roots(manual_roots)
    
    new_active_roots = manager.get_active_roots()
    print(f"   手动指定后的激活词根: {new_active_roots}")
    
    # 测试关键词生成
    print(f"\n🔄 测试关键词生成:")
    seed_words = ['image', 'text', 'video']
    combinations = manager.generate_keyword_combinations(seed_words, max_combinations=20)
    
    print(f"   生成的关键词组合 (前10个):")
    ...
    print(f"\n📈 测试词根覆盖分析:")
    test_keywords = [
        'ai image generator', 'pdf converter', 'text analyzer',
        'video editor', 'code optimizer', 'data processor'
    ]
    
    coverage = manager.analyze_root_coverage(test_keywords)
    print(f"   词根覆盖率: {coverage['coverage_rate']}%")
    print(f"   使用最多的词根: {coverage['top_used_roots'][:5]}")
    
    # 测试机会建议
    suggestions = manager.suggest_missing_opportunities(test_keywords)
    print(f"\n💡 建议的新关键词 (前5个):")
